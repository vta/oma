...
public class TableData {
    ...
    public void createNewFile(String date) {
        ...
        for(int i = 3; i < 1000; i++) {
            data.add(new ValueRange().setRange("Sheet1!A" + i + ":L" + i).setValues(values));
        }
        ...
        values = Arrays.asList(Arrays.asList("Run", "Block", "Coach", "Operator", "Time Due", "First Time", "Direction",
                "Last Time", "Pull-In Time", "Actual", "Outlate/Cancel/Pull", "Operator/Equipment"));
        body = new ValueRange().setValues(values);
        try {
            UpdateValuesResponse result =
                    service.spreadsheets().values().update(spreadsheetId, "Sheet1!A2:L2", body)
                            .setValueInputOption("RAW").execute();
        } catch(Exception e) {e.printStackTrace();}
    }
