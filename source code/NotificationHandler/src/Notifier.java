...
public class Notifier {
    ...
    public static void notifyPriorityGroup(Sheets service, int columnIndex) {
        String col = "";
        if(columnIndex == 1) col = "E";
        if(columnIndex == 2) col = "C:E";
        if(columnIndex == 3) col = "A:E";
        List<List<Object>> values;
        try {
            BufferedReader reader = new BufferedReader(new FileReader("Assets/PrioritySheetId.txt"));
        ...
        if(values != null) {
            String priority = "low";
            if(columnIndex == 2) priority = "medium";
            if(columnIndex == 3) priority = "high";
            for(List<Object> objects : values) {
                for (Object o : objects) {
                    System.out.println("notifying " + o);
                    try {
                        Process pr = rt.exec("node app.js " + o + " " + priority);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
